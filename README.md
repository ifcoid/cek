# Ceklist Kelengkapan dan Persiapan Sidang

Ceklist Persiapan Sidang:
1. Memahami konsep dasar algoritma. algoritma adalah latinisasi dari nama Al-Khawarizmi (Muhammad ibn Mūsā al-Khwārizmī), yang hidup sekitar abad ke-9.
2. Buatkan agenda menggunakan Google Calendar yang informatif bersama pembimbing dan penguji, baru kemudian menghubungi pembimbing dan penguji dengan whatsapp detil agenda beserta link Google Calendar
3. BAB I pada buku laporan pastikan merupakan hasil **Planning** dari parsif.al
4. BAB II pada buku laporan pastikan merupakan hasil **Conducting** dari parsif.al
5. Gunakan Latex untuk membuat Buku Laporan
6. Ceritakan Dampak pekerjaan atau penelitian ini untuk masyarakat
7. Jangan baca slide, ukuran font minimal 24, perbanyak ilustrasi dan infografis bukan copas isi laporan.


Dokumentasi yang **wajib ada** berdasarkan yang dikerjakan:
* API : Swagger
* Kode Program : git SCM(Source Code Manajemen)
* Dokumen : Latex

Daftar Link dan Kontak Bantuan:
1. [Template Latex D4TI](https://github.com/saripudin14/latex) : muhammad arif rivaldi 714240008 a.k.a Saripudin +62 813-8262-6096
2. Tata cara penggunaan Google Calendar : Bila Ismina +62 895-3297-46650
3. Tata cara penggunaan Parsifal : Balqis dan Sindi +62 851-5957-7009 - +62 831-9580-0022
